...
function test(fileName, steps, Gammas, n, per, KPerc, rhos)
    ...
    model0, dict0, obj0 = nominal(c, [b; -b; u], [A; -A; identity0], false, false)
    time = @elapsed nominal(c, [b; -b; u], [A; -A; identity0], false, false)
    ...
    for i in 1:steps
        ...
        model01, dict01, obj01 = nominal(cWorst, [b; -b; u], [A; -A; identity0], false, false)
        time = @elapsed nominal(cWorst, [b; -b; u], [A; -A; identity0], false, false)
        write(fNomWorst, string(obj01) * " " * string(time) * "\n")
         ...
         for Gamma in Gammas
            ...
            model1, dict1, obj1 = onestageMin(c1, spzeros(n*n + 1), [u; 100000000000000000],
            sparse([0; b; -b]), A1, Gamma1, J1, cU1, true, false, false)
            time = @elapsed onestageMin(c1, spzeros(n*n + 1), [u; 100000000000000000],
            sparse([0; b; -b]), A1, Gamma1, J1, cU1, true, false, false)
            ...
            for j in 1:length(rhos)
                model2, dict2, obj2 =  lightRobustnessMin(c1, sparse([0; b; -b; u]), sparse([A1; identity]), Gamma2,
                cU2, rhos[j], false, false, false)
                time = @elapsed lightRobustnessMin(c1, sparse([0; b; -b; u]), sparse([A1; identity]), Gamma2,
                cU2, rhos[j], false, false, false)
            ...
            for j in 1:length(KPerc)
                ...
                model3, dict3, obj3 = recoverableMin(sparse([c; spzeros(n*n)]), spzeros(2 * n * n),
                sparse([cU; spzeros(n * n)]), sparse([b; u]), sparse([A spzeros(n, n*n); identity0 identity0]), Gamma3, K, false, false)

                time = @elapsed recoverableMin(sparse([c; spzeros(n*n)]), spzeros(2 * n * n),
                sparse([cU; spzeros(n * n)]), sparse([b; u]), sparse([A spzeros(n, n*n); identity0 identity0]), Gamma3, K, false, false)
                constraints = checkConstraints(A, [], b, dict3, n * n, 0, 2)

                model4, dict4, obj4 = recoverableMinInf(sparse([c; spzeros(n*n)]), spzeros(2 * n * n),
                sparse([cU; spzeros(n * n)]), sparse([b; u]), sparse([A spzeros(n, n*n); identity0 identity0]), Gamma3, K, false, false)

                time = @elapsed recoverableMinInf(sparse([c; spzeros(n*n)]), spzeros(2 * n * n),
                sparse([cU; spzeros(n * n)]), sparse([b; u]), sparse([A spzeros(n, n*n); identity0 identity0]), Gamma3, K, false, false)
