...
function test(fileName, steps, Gammas, per, rhos, T, n)
    ...
    for s in 1:steps
        ...
        if s == 1
            ...
            # nominal
            model0, dict0, obj0 = nominal(c, [Cap; vmNom; vmaNom; P], ANom, false, false)
            time = @elapsed nominal(c, [Cap; vmNom; vmaNom; P], ANom, false, false)
        ...
        model01, dict01, obj01 = nominal(c, [Cap; vmNomW; vmaNomW; P], ANom, false, false)
        time = @elapsed nominal(c, [Cap; vmNomW; vmaNomW; P], ANom, false, false)
        ...
        for g in Gammas
            ...
            model1, dict1, obj1 = onestageMin(sparse([c; 0]), [], [], b, A, Gamma, J, bMU, false, false, false)
            time = @elapsed onestageMin(sparse([c; 0]), [], [], b, A, Gamma, J, bMU, false, false, false)
            constraints = checkConstraints(A, [], b, dict1,  n * T + 1, 0, 0)
            write(fMinMax, string(g) * " " * string(obj1) * " " * string(constraints) * " " * string(time) * "\n")

            # light robustness
            for j in 1:length(rhos)
                model2, dict2, obj2 =  lightRobustnessMin(sparse([c; 0]), b, A, Gamma, [z bMU], rhos[j], false, false, false)
                time = @elapsed lightRobustnessMin(sparse([c; 0]), b, A, Gamma, [z bMU], rhos[j], false, false, false)
            ...
            for p in 1:length(infos)
                ...
                model3, dict3, obj3 = adjustableMinB(sparse([zeros(n); [1]; [0]]), bA, AA, B, g, bAU, zeroQ1, false, false)
                time = @elapsed adjustableMinB(sparse([zeros(n); [1]; [0]]), bA, AA, B, g, bAU, zeroQ1, false, false)
                constraints = checkConstraints(AA, B, bA, dict3, n + 2, (n) * (T - 1), 2)
                write(fAdj, string(g) * " " * string(obj3) * " " * string(constraints) * " " * string(time) * " ")
            end
            model3, dict3, obj3 = adjustableMinB(sparse([zeros(n); [1]; [0]]), bA, AA, B, g, bAU, zeroQ, false, false)
            time = @elapsed adjustableMinB(sparse([zeros(n); [1]; [0]]), bA, AA, B, g, bAU, zeroQ, false, false)
            constraints = checkConstraints(AA, B, bA, dict3, n + 2, (n) * (T - 1), 2)
          write(fAdj, string(g) * " " * string(obj3) * " " * string(constraints) * " " * string(time) * " ")

            model3, dict3, obj3 = adjustableMinB(sparse([zeros(n); [1]; [0]]), bA, AA, B, g, bAU, [], false, false)
            time = @elapsed adjustableMinB(sparse([zeros(n); [1]; [0]]), bA, AA, B, g, bAU, [], false, false)
            constraints = checkConstraints(AA, B, bA, dict3, n + 2, (n) * (T - 1), 2)
          write(fAdj, string(g) * " " * string(obj3) * " " * string(constraints) * " " * string(time) * "\n")
