...
# 104264919559604445
# 4937843207976251516
# 6048072713530191983 złe zachowanie
# periods
T = 24
# no of factories
n = 5
...
for r in 1:length(dU)
    dU[r] = rand(Uniform(0.5 * d[r], d[r] * per))
end
...
#      end
# end
# println(stderr, zeroQ)
vm = [-VMin + v1 for i in 1:T]
vma = [VMax - v1 for i in 1:T]
b  = sparse([Cap; vm; vma; P; [1]; [-1]])
bA = sparse([0; Cap; -dSum; dSum; P; [1]; [-1]])
bAU = sparse([spzeros(1 + length(Cap)); -dUSum; dUSum; spzeros(n*T); 0; 0])
model, dict, opt = adjustableMinB(sparse([zeros(n); [1]; [0]]), bA, AA, B, 1.0, bAU, zeroQ, false, false)
...
model, dict, opt = robustOpt.minmax(sparse([c; 0]), [], [], b, A, Gamma, J, bMU, false, false, false)
...
println(stderr, "nominal " * string(obj0) * " " * string(time))
